...
export function DoctorList({ reloadSignal }: { reloadSignal?: number } = {}) {
  const [doctors, setDoctors] = useState<Doctor[]>([]);
  const [error, setError] = useState("");
  const [search, setSearch] = useState("");
  const [categoryFilter, setCategoryFilter] = useState<"ALL" | "VIP" | "A" | "B">("ALL");
  ...
  const filteredDoctors = doctors.filter((doctor) => {
    ...
    const matchesSearch =
      ...
      doctor.territory.toLowerCase().includes(query);
    const matchesCategory =
      categoryFilter === "ALL" ||
      (categoryFilter === "VIP" && doctor.category === "A") ||
      (categoryFilter === "A" && doctor.category === "A") ||
      (categoryFilter === "B" && doctor.category === "B");
    return matchesSearch && matchesCategory;
  });

  const visibleDoctors = sorted
  ...
  return (
    <div className="space-y-4">
      {toast && (
        <div className="fixed top-4 inset-x-4 max-w-sm mx-auto z-50 flex items-center gap-2.5 px-3.5 py-2.5 rounded-xl bg-slate-900/95 text-white text-xs font-medium shadow-xl backdrop-blur-md border border-slate-800">
          <span>{toast}</span>
      ...
      <div className="flex items-center space-x-2 overflow-x-auto no-scrollbar py-0.5 -mx-4 px-4">
        <button
          className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold ${categoryFilter === "ALL" ? "bg-brand-700 text-white shadow-sm" : "bg-white text-slate-700 border border-slate-200"}`}
          onClick={() => setCategoryFilter("ALL")}
        >
          All ({doctors.length})
        </button>
        <button
          className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold ${categoryFilter === "VIP" ? "bg-brand-700 text-white shadow-sm" : "bg-amber-50 text-amber-900 border border-amber-200 hover:bg-amber-100"}`}
          onClick={() => setCategoryFilter("VIP")}
        >
          ★ VIP KOL ({doctors.filter(d => d.category === 'A').length})
        </button>
        <button
          className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold ${categoryFilter === "A" ? "bg-brand-700 text-white shadow-sm" : "bg-white text-slate-700 border border-slate-200 hover:bg-slate-50"}`}
          onClick={() => setCategoryFilter("A")}
        >
          Class A ({doctors.filter(d => d.category === 'A').length})
        </button>
        <button
          className={`shrink-0 px-3 py-1.5 rounded-full text-xs font-semibold ${categoryFilter === "B" ? "bg-brand-700 text-white shadow-sm" : "bg-white text-slate-700 border border-slate-200"}`}
      ...
      <div className="p-3.5 bg-gradient-to-r from-emerald-900 to-brand-800 rounded-2xl text-white shadow-sm flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center space-x-2">
              <h3 className="font-bold text-xs tracking-tight text-white">{visibleDoctors.length} Visits Remaining</h3>
        ...
        <button
          className={`shrink-0 text-xs font-bold px-3 py-1.5 rounded-lg shadow-sm active:scale-95 transition-all ${sorted ? "bg-brand-900 text-white" : "bg-white text-brand-800 hover:bg-emerald-50"}`}
              ...
              <div className="mt-3.5 pt-3 border-t border-slate-100 flex items-center justify-between">
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    title="Call Doctor"
                    onClick={() => showToast("No phone number on file for this doctor")}
                  >
                  ...
                  <button
                    ...
                    onClick={() => window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(doctor.name + " " + doctor.city + " " + doctor.state)}`, "_blank", "noopener,noreferrer")}
                  >
